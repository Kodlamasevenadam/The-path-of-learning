...
class IntegralApp:
    def __init__(self, master):
        ...
        # Giriş alanları
        tk.Label(master, text="Fonksiyon (x veya y):").grid(
    ...
    def calculate(self):
        ...
        try:
            if var_str not in ["x", "y"]:
                raise ValueError(
                    "Sadece 'x' veya 'y' değişkeni kullanılabilir.")

            var = sp.Symbol(var_str)
            func = sp.sympify(func_str)

            # Diğer değişken varsa uyar
            other_var = "y" if var_str == "x" else "x"
            if sp.Symbol(other_var) in func.free_symbols:
                raise ValueError(
                    "Grafik çizimi yalnızca tek değişkenli fonksiyonlar için destekleniyor.")
